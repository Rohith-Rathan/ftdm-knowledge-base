...
if [ ! -f "Stage5_RTSD_Creation/validated_service_interfaces.md" ]; then
    echo "❌ ERROR: Validated service interfaces not found"
    echo "📋 REQUIRED: Validated service interfaces document in Stage5_RTSD_Creation/validated_service_interfaces.md"
    echo "💡 ACTION: Run Phase 7: Interface Validation & Coverage first"
    exit 1
fi

# Check if business logic requirements exist
if [ ! -f "Stage5_RTSD_Creation/business_logic_requirements.md" ]; then
    echo "❌ ERROR: Business logic requirements not found"
    echo "📋 REQUIRED: Business logic requirements document in Stage5_RTSD_Creation/business_logic_requirements.md"
    echo "💡 ACTION: Run Phase 7: Interface Validation & Coverage first"
    exit 1
fi
...
if [ ! -f "Stage5_RTSD_Creation/orm_mappings.md" ]; then
    echo "❌ ERROR: ORM mappings not found"
    echo "📋 REQUIRED: ORM mappings document in Stage5_RTSD_Creation/orm_mappings.md"
    echo "💡 ACTION: Run Phase 5: ORM Mapper & MongoDB Classes first"
    exit 1
fi

# Check if business rules exist
if [ ! -f "Stage5_RTSD_Creation/business_rules.md" ]; then
    echo "❌ ERROR: Business rules not found"
    echo "📋 REQUIRED: Business rules document in Stage5_RTSD_Creation/business_rules.md"
    echo "💡 ACTION: Run Phase 6: Business Service Interfaces first"
    exit 1
fi
